...
type FilterStatus = 'all' | 'starred' | 'completed' | 'pending' | 'failed' | 'cancelled';
type SortKey = 'orderCode' | 'payment' | 'amount' | 'currency' | 'status' | 'time';
...
function generateMockData(): Transaction[] {
    const gateways = ['MoMo', 'ZaloPay', 'Chuyển khoản', 'Thẻ Visa', 'Thẻ Mastercard', 'QR Code'];
    const statuses: StatusKey[] = ['COMPLETED', 'PENDING', 'FAILED', 'CANCELLED'];
    const amounts = [
        199000, 299000, 499000, 599000, 799000, 999000, 1299000, 1499000,
        1999000, 2499000, 2999000, 3999000, 4999000, 150000, 350000, 450000,
        690000, 890000, 1190000, 5999000, 7999000, 9999000,
    ];
    ...
    for (let i = 0; i < 37; i++) {
        ...
        const statusWeights = [0, 0, 0, 0, 0, 1, 1, 2, 3];
        const statusIdx = statusWeights[Math.floor(Math.random() * statusWeights.length)];
        ...
        mockTransactions.push({
            ...
            paymentGateway: gateways[Math.floor(Math.random() * gateways.length)],
            gatewayTransactionId: `TXN${String(1000000 + Math.floor(Math.random() * 9000000))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}`,
...
export default function OrdersPage() {
    ...
    const mockData = useRef<Transaction[]>(generateMockData()).current;
    ...
    useEffect(() => {
        ...
        if (status === 'authenticated') {
            fetch('/api/account/orders')
                ...
                .then(d => {
                    const data = d.data || [];
                    setTransactions(data.length > 0 ? data : mockData);
                })
                .catch(() => {
    ...
    // ── Filter options (matching Notifications style) ──
    const filterOptions: { key: FilterStatus; label: string; icon: React.ReactNode }[] = [
        ...
        { key: 'completed', label: t('orders.status.completed'), icon: <CheckCircle2 className="size-3.5 text-current" /> },
        { key: 'pending', label: t('orders.status.pending'), icon: <Clock className="size-3.5 text-current" /> },
        { key: 'failed', label: t('orders.status.failed'), icon: <AlertCircle className="size-3.5 text-current" /> },
        { key: 'cancelled', label: t('orders.status.cancelled'), icon: <XCircle className="size-3.5 text-current" /> },
    ];
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <Popover>
                        ...
                        >
                            ...
                            {/* Custom range with BirthDatePicker */}
            ...
            <Card className="page-card overflow-hidden">
                <CardContent className="p-0">
                    ...
                    <div className="hidden md:flex items-center gap-3 px-3 py-2.5 border-b border-border/40 dark:border-white/[0.08] bg-black/[0.02] dark:bg-white/[0.02]">
                        ...
                        >
                            {([
                                { key: 'orderCode' as SortKey, label: t('orders.col.orderCode'), align: '' },
