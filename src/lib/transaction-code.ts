...
// Re-export formatTransactionCode from the shared (client-safe) module
export { formatTransactionCode } from '@/lib/format-transaction-code';

/**
 * Transaction code format: TDDMMYYDDD (10 digits)
 *
...
export type TransactionType = 'tradingToolsAndData' | 'aiAndSoftware' | 'customSolutions' | 'news' | 'other';

const TYPE_PREFIX: Record<TransactionType, number> = {
...
/**
 * Generate a new transaction code in format TDDMMYYDDD.
...
export async function generateTransactionCode(
    ...
    // Count today's transactions to get daily sequential
