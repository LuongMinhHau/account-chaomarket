...
import { COOKIE_SIDEBAR_STATE_NAME } from '@/constant';
...
const SIDEBAR_FALLBACK: SidebarContextProps = {
    state: 'expanded',
    open: true,
    setOpen: () => {},
    openMobile: false,
...
function useSidebar() {
    ...
    if (!context) {
        // Return safe defaults instead of throwing — prevents SSR/bot crashes
        // when components using useSidebar() render before SidebarProvider hydrates
        ...
        <button
            ...
            className={cn(
                'hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all!' +
                    ' ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex',
...
function SidebarGroupLabel({
    ...
    return (
        <Comp
            ...
            className={cn(
                'text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-xs' +
                    ' font-medium outline-hidden transition-[margin,opacity]! duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
        ...
        <Comp
            ...
            className={cn(
                'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent' +
                    ' hover:text-sidebar-accent-foreground absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform! focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
    'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden' +
        ' ring-sidebar-ring transition-[width,height,padding]! hover:text-sidebar-accent-foreground hover:font-semibold dark:hover:bg-sidebar-accent focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
    {
        variants: {
            variant: {
                default:
                    'dark:hover:bg-sidebar-accent hover:text-sidebar-accent-foreground',
                outline:
...
function SidebarMenuAction({
    ...
    return (
        <Comp
            ...
            className={cn(
                'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent' +
                    ' hover:text-sidebar-accent-foreground peer-hover/menu-button:text-sidebar-accent-foreground absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform! focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
                // Increases the hit area of the button on mobile.
                'after:absolute after:-inset-2 md:after:hidden',
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
    return (
        <ul
            ...
            className={cn(
                'border-sidebar-border/50 mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l-[0.25px] px-2.5 py-0.5',
                'group-data-[collapsible=icon]:hidden',
                className
            )}
            {...props}
